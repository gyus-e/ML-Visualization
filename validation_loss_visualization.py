...
LOGS_DIR = 'logs'
FIGS_DIR = 'figures/validation_loss'
...
hidden_sizes = sorted(val_df['hidden_layer_size'].unique())

for FIXED_HIDDEN_SIZE in hidden_sizes:
    # Filter for the fixed hidden layer size
    size_df = val_df[val_df['hidden_layer_size'] == FIXED_HIDDEN_SIZE].copy()
    ...
    ax.set_ylabel('Average Validation Loss', fontsize=12, fontweight='bold')
    ax.set_title(f'Validation Loss over Epochs for Hidden Layer Size = {FIXED_HIDDEN_SIZE}\n(Averaged across seeds)', 
    ...
    # Print final validation loss for each combination
    print("\nFinal validation loss (at last epoch) for each combination:")
