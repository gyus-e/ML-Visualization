...
grouped = test_df.groupby(['hidden_layer_size', 'learning_rate', 'momentum'])['loss'].mean().reset_index()
...
for i, combo in enumerate(combinations):
    ...
    # Create a list of losses aligned with hidden_sizes
    losses = []
    for size in hidden_sizes:
        size_data = combo_data[combo_data['hidden_layer_size'] == size]
        if len(size_data) > 0:
            losses.append(size_data['loss'].values[0])
        else:
            losses.append(0)  # If no data for this combination
    
    # Calculate position for this group of bars
    positions = x + (i - n_combos/2 + 0.5) * bar_width
    
    # Plot the bars
    ax.bar(positions, losses, bar_width, label=combo, color=color, alpha=0.8)

# Customize the plot
ax.set_xlabel('Hidden Layer Size', fontsize=12, fontweight='bold')
ax.set_ylabel('Average Test Loss', fontsize=12, fontweight='bold')
ax.set_title('Average Test Loss by Hidden Layer Size and Hyperparameters\n(Averaged across seeds)', 
...
for i, combo in enumerate(combinations):
    ...
    for pos, loss in zip(positions, losses):
        if loss > 0:  # Only show label if there's data
            ax.text(pos, loss + 0.02, f'{loss:.3f}', ha='center', va='bottom', 
                   fontsize=7, rotation=90) # TODO: CAMBIARE rotation A 0 PER ORIZZONTALE

plt.tight_layout()
plt.savefig(os.path.join(FIGS_DIR, 'test_loss.png'), dpi=300, bbox_inches='tight')
print(f"Visualization saved to: {os.path.join(FIGS_DIR, 'test_loss.png')}")
print(f"\nData summary:")
